...
#if defined(NDEBUG)
# error "Livre cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x27856e427fa70e01dde68276a2f4509b9e2a753b3cb4673ce418833a52c909c9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Livre: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Livre Signed Message:\n";
...
{
    // Livre: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 940000); // Livre: 940k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.5 * 24 * 60 * 60; // Livre: 3.5 days
static const int64 nTargetSpacing = 1.5 * 60; // Livre: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Livre: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Livre: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xfb7b723d09767ae9ac471d8fd48fc278ae48b7dd8eeea028aab8e90d4e575fff");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Br SP. 11/Nov/2020 Criada a moeda Livre, LIVRECOIN, 15:32";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d2582f6948b616df614a398d4cecd56b271219a14725e79c8196edde8751d5e5b9792e8bf06e0e7aeb57d669ae538051ed1726ee16ade94199d07710f9e2142e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1605120943;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086201538;

        if (fTestNet)
        {
            block.nTime    = 1605120895;
            block.nNonce   = 385856792;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x6b2e90502f2ad2188ba7d74a62740c9cbddd421247c2ac34c5f165ef31eef65e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xaf, 0xf1, 0xa1, 0xcd }; // Livre: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LivreMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LivreMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LivreMiner(CWallet *pwallet)
{
    printf("LivreMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("livre-miner");
    ...
    try { loop {
        ...
        printf("Running LivreMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LivreMiner, pwallet));
}
